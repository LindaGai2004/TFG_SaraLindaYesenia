...
export default function TrabajadoresJefe({
  trabajadores,
  onAddTrabajador,
  onEditTrabajador,
  onDeleteTrabajador,
  ...
  const filtered = useMemo(() =>
    (trabajadores ?? []).filter(t =>
      `${t.nombre} ${t.apellidos} ${t.email} ${t.username}`
        .toLowerCase()
    ...
    <div>
      ...
      {formModal && (
        <Modal
          ...
          title={formModal.type === 'add' ? '➕ Añadir Trabajador' : '✏️ Editar Trabajador'}
        >
          <StaffForm
            ...
            onSave={d => {
              formModal.type === 'add'
                ? onAddTrabajador(d)
                : onEditTrabajador(formModal.data.email, d);
              setFormModal(null);
            }}
          />
        </Modal>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="form-group">
        ...
        <input value={form.direccion} onChange={e => set('direccion', e.target.value)} className="input-field" />
      </div>
